  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all">
      ...
      <div className="p-4">
        {post.caption && (
          <div>
            <p className={`text-sm text-slate-700 ${expanded ? '' : 'line-clamp-2'}`}>{post.caption}</p>
          ...
          <div className="mt-3 pt-3 border-t border-slate-100 grid grid-cols-4 gap-1 text-center">
            {[
              ...
              <div key={label}>
                ...
                <div className="text-xs font-bold text-slate-700">{fmtNum(val)}</div>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-0.5">Your published content and performance overview</p>
      </div>
      ...
      <div className="flex gap-1 bg-slate-100 rounded-lg p-1 w-fit">
        {[{ key: 'feed', label: 'Content Feed' }, { key: 'breakdown', label: 'Platform Breakdown' }].map(({ key, label }) => (
          <button key={key} onClick={() => setTab(key)}
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${tab === key ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}>
            {label}
          </button>
        ))}
      </div>
          ...
          {platforms.length > 1 && (
            <div className="flex gap-2 flex-wrap">
              <button onClick={() => setPlatform('')}
                className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${platform === '' ? 'bg-slate-800 text-white border-slate-800' : 'border-slate-200 text-slate-600 hover:border-slate-300'}`}>
                All Platforms
              </button>
              {platforms.map(p => {
                ...
                return (
                  <button key={p} onClick={() => setPlatform(p)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${platform === p ? `${pm.bg} ${pm.text} border-current` : 'border-slate-200 text-slate-600 hover:border-slate-300'}`}>
                    <PIcon size={10} />{pm.label}
                  </button>
                );
              })}
            ...
            return (
              <div key={p._id} className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <div className="font-semibold text-slate-800">{pm.label}</div>
                    <div className="text-xs text-slate-500">{p.posts} posts this month</div>
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-3">
                  {[
                    ...
                    <div key={label} className="bg-slate-50 rounded-lg p-2 text-center">
                      <div className="text-sm font-bold text-slate-800">{fmtNum(val)}</div>
