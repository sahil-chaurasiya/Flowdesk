...
export default function ClientLeadsPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-xl font-bold text-slate-800">🎯 Your Leads</h1>
        <p className="text-slate-500 text-sm mt-0.5">All leads generated from your campaigns, uploaded by your team.</p>
      </div>
      ...
      {stats?.bySource?.length > 0 && (
        <Card className="p-4">
          <div className="text-sm font-semibold text-slate-800 mb-3">Leads by Source</div>
          <div className="flex flex-wrap gap-2">
            {stats.bySource.map(s => (
              <div key={s._id} className="flex items-center gap-2 bg-slate-50 border border-slate-200 px-3 py-2 rounded-xl text-xs">
                <span className="font-semibold text-slate-700">{s._id || 'Unknown'}</span>
            ...
            <Card key={batch._id} className="overflow-hidden">
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="font-semibold text-slate-900">{batch.batchLabel || 'Lead Upload'}</div>
                    <div className="text-xs text-slate-400 mt-0.5 flex items-center gap-2">
                      ...
                      {batch.sources?.filter(Boolean).length > 0 && (
                        <>
                          <span>·</span>
                          <span className="text-slate-500">{batch.sources.filter(Boolean).join(', ')}</span>
                        </>
                      )}
                    </div>
                  </div>
              ...
              {expandedBatch === batch._id && (
                <div className="border-t border-slate-100">
                  {leads.length === 0 ? (
                    <div className="py-8 text-center text-slate-400 text-sm">Loading leads...</div>
                  ) : (
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {leads.map(lead => (
                            <tr key={lead._id} className="hover:bg-slate-50 transition-colors">
                              <td className="px-4 py-3 font-medium text-slate-800 text-xs">{lead.name || '—'}</td>
                              <td className="px-4 py-3 text-slate-600 text-xs">{lead.email || '—'}</td>
                              <td className="px-4 py-3 text-slate-600 text-xs">{lead.phone || '—'}</td>
                              <td className="px-4 py-3 text-slate-600 text-xs">{lead.company || '—'}</td>
                              ...
                              <td className="px-4 py-3 text-slate-400 text-xs">{formatDate(lead.leadDate || lead.createdAt)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
